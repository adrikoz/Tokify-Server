            ...
            if (record.eventName == 'INSERT' || record['dynamodb']['OldImage']['timestamp']['S'] !== record['dynamodb']['NewImage']['timestamp']['S']) {
                try {
                    ...
                    if (record['dynamodb']['NewImage']['parameters']['M']['selectedFunctions']['S'] === "Liquidity Generator"){
                        ...
                        console.log("safemoon: ", JSON.stringify(data));
                        const content = data.Body.toString('ascii').split("\n");
                        content.splice(438, 0, `contract ${nameNoSpace} is IBEP20, Auth {`);
                        content.splice(442, 0, `    address WBNB = ${params["routerBaseToken"]["S"]};`);
                        content.splice(447, 0, `    string constant _name = \"${params["tokenName"]["S"]}\";`);
                        content.splice(448, 0, `    string constant _symbol = \"${params["tokenSymbol"]["S"]}\";`);
                        content.splice(449, 0, `    uint8 constant _decimals = ${params["decimals"]["N"]};`);
                        content.splice(451, 0, `    uint256 private _totalSupply = ${params["totalSupply"]["N"]};`);
                        content.splice(462, 0, `    uint256 liquidityFee = ${params["transactionLiquidity"]["N"]};`);
                        content.splice(464, 0, `    uint256 reflectionFee = ${params["rewardsBase"]["N"]};`);
                        content.splice(466, 0, `    uint256 marketingFee = ${params["marketingFee"]["N"]};`);
                        content.splice(500, 0, `        address _dexRouter = ${params["router"]["S"]};`);
                        ...
                        console.log("safemoon: ", JSON.stringify(data));
                        const content = data.Body.toString('ascii').split("\n");
                        content.splice(230, 0, `contract ${nameNoSpace} is IBEP20, Auth {`);
                        content.splice(234, 0, `    address public WBNB = ${params["routerBaseToken"]["S"]};`);
                        content.splice(239, 0, `    string constant _name = \"${params["tokenName"]["S"]}\";`);
                        content.splice(240, 0, `    string constant _symbol = \"${params["tokenSymbol"]["S"]}\";`);
                        content.splice(241, 0, `    uint8 constant _decimals = ${params["decimals"]["N"]};`);
                        content.splice(243, 0, `    uint256 private _totalSupply = ${params["totalSupply"]["N"]};`);
                        content.splice(256, 0, `    uint256 buyLiquidityFee = ${params["buyLiquidityFee"]["N"]};`);
                        content.splice(257, 0, `    uint256 buyMarketingFee = ${params["buyMarketingFee"]["N"]};`);
                        content.splice(258, 0, `    uint256 buyDeveloperFee = ${params["buyDeveloperFee"]["N"]};`);
                        content.splice(259, 0, `    uint256 buyTeamFee = ${params["buyTeamFee"]["N"]};`);
                        content.splice(262, 0, `    uint256 sellLiquidityFee = ${params["sellLiquidityFee"]["N"]};`);
                        content.splice(263, 0, `    uint256 sellMarketingFee = ${params["sellMarketingFee"]["N"]};`);
                        content.splice(264, 0, `    uint256 sellDeveloperFee = ${params["sellDeveloperFee"]["N"]};`);
                        content.splice(265, 0, `    uint256 sellTeamFee = ${params["sellTeamFee"]["N"]};`);
                        text = content.join("\n");
                        console.log("text: ", text);
                    } else {
