        ...
        if (record.eventName != 'REMOVE'){
            if (record.eventName == 'INSERT' || record['dynamodb']['OldImage']['timestamp']['S'] !== record['dynamodb']['NewImage']['timestamp']['S']) {
                try {
                    ...
                    } else if (record['dynamodb']['NewImage']['parameters']['M']['selectedFunctions']['S'] === "Testudo") {
                        const Key = 'lg_rBNB_mw_ap.sol';
                        console.log("point 1");
                        const data = await s3.getObject({ Bucket, Key }).promise();
                        console.log("safemoon: ", JSON.stringify(data));
                        const content = data.Body.toString('ascii').split("\n");
                        content.splice(171, 0, `    address public WBNB = ${params["routerBaseToken"]["S"]};`);
                        content.splice(211, 0, `        : IDEXRouter(${params["router"]["S"]});`);
                        content.splice(385, 0, `contract ${nameNoSpace} is IBEP20, Auth {`);
                        content.splice(389, 0, `    address WBNB = ${params["routerBaseToken"]["S"]};`);
